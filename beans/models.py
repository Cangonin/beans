...
class ResNetClassifier(nn.Module):
    def __init__(
        self, model_type, pretrained=False, num_classes=None, multi_label=False
    ):
        super().__init__()
...
class HubertClassifier(nn.Module):
    ...
    def forward(self, x, y=None):
        out, _ = self.hubert_base(
            x
        )  # Will get dimension [batch size, time, num_features]
...
class SingleMultiTaskEncoder(torch.nn.Module):
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            x = torch.mean(x, dim=1)  # output shape (batch size, hidden_size)
...
# TODO: see if I need to normalise the input or do something else
class HuBERTMTLClassifierFrozen:
    def __init__(self, model_path: str, num_classes=None, multi_label=False):
        models, _, _ = load_model_ensemble_and_task([model_path])
        self.hubert_mtl = models[0]

        self.hubert_mtl.eval()
        self.linear = nn.Linear(
            in_features=768, out_features=num_classes
        )  # TODO: check it's correct dimension
    ...
    def forward(self, x: torch.Tensor, y=None):
        with torch.no_grad():
            x = self.hubert_mtl(
                source=x,
                is_item_annotated=[True for _ in range(x.size(0))],
                target_list=[],  # for ssl samples only
                padding_mask=None,
                mask=False,
                features_only=True,
            )
        x = torch.mean(x["features"], dim=1)
        logits = self.linear(x)
        loss = None
        if y is not None:
            loss = self.loss_func(logits, y)
